...
public class PatientsLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(details2 != null) {
			System.out.println("Patient's Login Success !");
			HttpSession session = request.getSession();
			session.setAttribute("patientdetails2", details2);
			response.sendRedirect("../patients_records_maintenance/Patients/PatientsDashboard.jsp");
		}
		else {
			System.out.println("Patient's Login Failed");
			HttpSession session = request.getSession();
			session.setAttribute("login_failed", "Please check patient's login credentials");
